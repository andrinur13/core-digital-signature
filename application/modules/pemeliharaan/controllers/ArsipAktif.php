...
class ArsipAktif extends Dashboard_Controller {

	private $path = 'pemeliharaan';
	private $master = 'master';
    private $mod = 'ArsipAktif';
	...
	function __construct() 
	{
		...
		$this->load->model($this->path.'/m_arsip_aktif');
	...
	public function index() {
        ...
        $data['KlasifikasiList']= $this->m_arsip_aktif->get_klasifikasi();
        $data['GetAllBerkas']= $this->m_arsip_aktif->get_berkas(get_user_unit_id());

		// dd($data);

        $this->template->title('Arsip Aktif');
		$this->template->set_breadcrumb('Arsip Aktif', $this->module);
	...
	private function get_surat($data){
		// dd($data);
		$surat = $this->m_arsip_aktif->cari_surat_by_nomor($data);
	...
	private function datatables_arsip($arsipId){
        ...
        $qry = $this->m_arsip_aktif->get_data_arsip($object, $length, $this->input->post('start'), $order, NULL);
        $iTotalRecords = (!is_null($qry)) ? intval($this->m_arsip_aktif->get_data_arsip($object, NULL, NULL, array(), 'counter')) : 0;
	...
	private function datatables(){
        ...
        $length = ($this->input->post('length') == -1) ? NULL : $this->input->post('length');
        $qry = $this->m_arsip_aktif->get_data($object, $length, $this->input->post('start'), $order, NULL);
        $iTotalRecords = (!is_null($qry)) ? intval($this->m_arsip_aktif->get_data($object, NULL, NULL, array(), 'counter')) : 0;
        $iDisplayStart = intval($this->input->post('start'));
        $sEcho = intval($this->input->post('draw'));
        $records = array();
        $records["data"] = array(); 
	...
	private function genereateBerkasNumber($klasId){
		...
		$lastNumber = $this->m_arsip_aktif->getLastNumberBerkas($params);
		...
		$detailKlasifikasi = $this->m_arsip_aktif->getDetailKlaifikasi($klasifikasi);
		...
		if(!empty($lastNumber)){
			if($lastNumber->brksNomor != ''){
				...
				if ($length == 1) {
					$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/000'.$num; // menulis kode spmu
				}else if ($length == 2) {
					$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/00'.$num; // menulis kode spmu
				}else if ($length == 3) {
					$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/0'.$num; // menulis kode spmu
				}else if ($length == 4) {
					$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/'.$num; // menulis kode spmu
				}
			}else{
				$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/0001';
			}
		}else{
			$BerkasNumber = 'ARS/'.get_user_unit_kode().'/'.$detailKlasifikasi->klasKode.'/'.$detailKlasifikasi->jnsklasKode.'/0001';
		}
	...
	public function update($encId = NULL){
		...
		if ($this->input->post('action') == 'submit') {
			...
			if(is_null( $get_data = $this->m_arsip_aktif->get_data(array('brksId' => ' = '. decode($encId) ), NULL, NULL, NULL, NULL) )) show_404();
			$data['module']     = $this->module;
	...
	public function arsip($encId = ''){
        ...
        $data['GetAllBerkas']= $this->m_arsip_aktif->get_berkas(get_user_unit_id());
        $data['ArsipId']= $encId;
        $data['detail_berkas']= $this->m_arsip_aktif->getDetailBerkas(decode($encId));
	...
	public function add_arsip($encId = ''){
		...
		if ($this->input->post('action') == 'submit') {
			...
			// if(is_null( $get_data = $this->m_arsip_aktif->get_data_arsip(array('arsBerkasId' => ' = '. decode($encId) ), NULL, NULL, NULL, NULL) )) show_404();
			$data['module']     = $this->module;
			$data['data_arsip']       = $this->m_arsip_aktif->get_list_arsip_internal();
			$data['jenis_exemplar']= $this->m_arsip_aktif->get_jenis_exemplar();
	...
	public function arsip_detail($encId = ''){
		...
		if (!$this->input->is_ajax_request()) exit('No direct script access allowed');
		$data['module']     = $this->module;
			$data['data']       = $this->m_arsip_aktif->get_detail_arsip_surat(decode($encId));
			$dt_referensi_surat = $this->m_arsip_aktif->get_data_referensi_arsip_surat(decode($encId));
...
/* End of file ArsipAktif.php */
/* Location: D:\laragon\www\bsi-devel\uad\e-office-uad\application\modules\pemeliharaan\controllers\ArsipAktif.php */