			...
			<table class="table table-separated table-striped" id="datatables_ajax" cellspacing="0">
				<thead class="bg-color-primary1">
					<tr>
						...
						<th class="font-weight-bold">Perihal Surat</th>
						<?php if ($isStaff == 'staff') { ?>
							<th class="font-weight-bold">Catatan Disposisi</th>
						<?php } ?>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$dt = $('#datatables_ajax').DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url($module . '/datatables?tipe=' . $isStaff); ?>",
				"type": "POST",
				"data": function(d) {
	...
	function detail(id, staff) {
		module = "<?= site_url($module . '/detail?id=') ?>" + id + '&tipe=' + staff;
		Modal({
			url: module,
