...
class Authentication
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			$users_default = $this->ci->users->$get_user_func($login);

			
			if(is_null($users_default)){
				$portal =  $this->portal_login($login, $password);
				// print_r($portal);die;
				if($portal->status == FALSE){
					$this->error = array('login' => $portal->errors);
				}else{
					if(!empty($portal->data)){
						$user =  $this->portal_login($login, $password);
					}else{
						$user = '';
					}
				}
			...
			if (!empty($user)) {	// login ok
				// cek apakah ini jenisnya login portal atau bukan
				
				$login_type = (isset($user->login)) ? $user->login : 'default';
				...
				}elseif($login_type == 'portal'){
					if ($user->data->is_banned == 1) {									// fail - banned
						$this->error = array('banned' => 'Banned By Portal');
					} else {
						$qry_privilage = $this->ci->user_privilage->get_privilage_menu(array('GroupDetailGroupId' => $user->data->group_id));
						...
						$this->ci->session->set_userdata(array(
								...
								'any_user_group_exist'	=> $this->is_any_user_group_exist($user->data->group_id),
								'menu_privilege'		=> $this->menu_privilege($qry_privilage),
								'action_privilege'		=> $this->action_privilege($qry_privilage),
								'user_real_name'		=> $user->data->realname,
								'is_logged_in' 			=> TRUE,
								'user_role_id'          => $user->data->role_id,
								'user_unit_kode'        => $user->data->unit_kode,
								'user_unit_id'          => $user->data->unit_id,
								'user_unit_name' 	=> $user->sysUnitKerja
						));

						if ($remember) {
							$this->create_autologin($user->data->username);
						}
						$this->clear_login_attempts($login);
						return FALSE;
					}
	...
	function portal_login($username, $password){
			$url = $this->ci->config->item('service_portal'); 
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, 
				array(
					'U4D-API-KEY:'.$this->ci->config->item('key_portal')
				)
			); 
			...
			$status = FALSE;
			if ($json->status) {
				$fak_kode = $this->ci->config->item('fak_kode');
				$users = $json->userdata;
				if(in_array($users->fak_kode, $fak_kode)){
					$status = TRUE; 
					$user = $users; 
					$error = 'valid';
				}else{
					$status = FALSE; 
					$error = 'Anda Tidak DiIzinkan Masuk';
				}
			}else{
			...
			$return = (object)array('status'=> $status , 'data' => $user , 'errors' => $error,'login' => 'portal');
			return $return;
	...
	function logout()
	{
		...
		$this->ci->session->unset_userdata(array('any_user_group_exist' , 'user_id' , 'user_email' , 'user_name' , 'user_real_name' , 'user_group' ,'user_group_name', 'is_logged_in', 'menu_privilege', 'action_privilege','user_role_id','user_unit_kode','user_unit_id','user_unit_name'));
	}
	...
	{
		return $this->ci->session->userdata('user_group_name');
	}
	...
	function get_user_unit_name()
	{
		return $this->ci->session->userdata('user_unit_name');
	}
	...
	{
		return $this->ci->session->userdata('user_app');
	}
	...
	function menu_privilege($qry)
	{
		...
		if ( !is_null( $qry ) ) {
            ...
            if($row->MenuIsShow == 'Ya'){
				$data_menu[$row->MenuId] = array('MenuId'=> $row->MenuId, 'MenuName'=> $row->MenuName, 'MenuParentId'=> $row->MenuParentId, 'MenuModule'=> $row->MenuModule, 'MenuIconClass'=> $row->MenuIconClass, 'MenuOrder'=> $row->MenuOrder,'MenuStyle'=> $row->MenuStyle,);
				}
			}
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						$this->ci->session->set_userdata(array(
								...
								'user_unit_kode'        => $user->UnitKode,
								'user_unit_id'          => $user->UserUnitId,
								'user_unit_kerja' 	=> $user->sysUnitKerja
						));
	...
	// 			$child = $this->build_cild_custom_menu($data, $value['MenuId']);
	// 			if( $child ){
	// 				if( $value['MenuStyle'] == 1 AND $parrent == 0){
	...
	// 								</ul>
	// 							';
	// 				} else if( $value['MenuStyle'] == 2 AND $parrent == 0){
	// 					$str .= '<li class="menu-item">
	// 							   <a class="menu-link" href="#">
	// 								  <span class="icon fa '. $value['MenuIconClass'] .'"></span>
	// 								  <span class="title">'. $value['MenuName'] .'</span>
	...
	// 							   </ul>
	// 							</li>';
	// 				} else if( $value['MenuStyle'] == 3 AND $parrent == 0){
	// 					$str .= '<li class="menu-item">
	// 							   <a class="menu-link" href="#">
	// 								  <span class="icon fa '. $value['MenuIconClass'] .'"></span>
	// 								  <span class="title">'. $value['MenuName'] .'</span>
	...
	// 				}	
	// 			} else {
	// 						if(!empty($value['MenuIconClass'])){
	// 								$str .= '
	// 									<li class="menu-item">
	// 										<a href="'. site_url( $value['MenuModule']  ) .' " class="menu-link">
	// 											<span class="icon fa '. $value['MenuIconClass'] .'"></span>
	// 											<span class="title">'. $value['MenuName'] .'</span>
	// 										</a>
	// 									</li>
